...
export function captureCartoonFrame(video: HTMLVideoElement, canvas: HTMLCanvasElement) {
  ...
  const levels = 6; const step = 255 / (levels - 1); const gamma = .82; const exposure = 1.08; const saturation = 1.2;
  ...
  const luminanceAt = (index: number) => source[index] * .2126 + source[index + 1] * .7152 + source[index + 2] * .0722;
  ...
  for (let index = 0; index < data.length; index += 4) {
    ...
    const pixel = index / 4; const x = pixel % canvasWidth; const y = Math.floor(pixel / canvasWidth);
    const left = ((y * canvasWidth) + Math.max(0, x - 1)) * 4;
    const right = ((y * canvasWidth) + Math.min(canvasWidth - 1, x + 1)) * 4;
    const above = ((Math.max(0, y - 1) * canvasWidth) + x) * 4;
    const below = ((Math.min(canvasHeight - 1, y + 1) * canvasWidth) + x) * 4;
    const neighborLuminance = (luminanceAt(left) + luminanceAt(right) + luminanceAt(above) + luminanceAt(below)) / 4;
    const edgeInk = Math.min(20, Math.abs(luminanceAt(index) - neighborLuminance) * .55);
    const luminance = red * .2126 + green * .7152 + blue * .0722;
    data[index] = posterize(clamp(luminance + (red - luminance) * saturation - edgeInk));
    data[index + 1] = posterize(clamp(luminance + (green - luminance) * saturation - edgeInk));
    data[index + 2] = posterize(clamp(luminance + (blue - luminance) * saturation - edgeInk));
  }
