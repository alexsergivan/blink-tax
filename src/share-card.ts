...
async function drawEvidence(c: CanvasRenderingContext2D, mugshot: string | null | undefined, receiptId?: string) {
  const x = 700; const y = 690; const size = 320;
  c.fillStyle = '#111'; c.fillRect(x, y, size, 390);
  c.strokeStyle = '#ffed00'; c.lineWidth = 7; c.strokeRect(x + 4, y + 4, size - 8, 382);
...
export async function generateShareCard({ seconds, isNewBest, best, url, mugshot, receiptId }: ShareCardData): Promise<Blob> {
  ...
  const score = `${seconds.toFixed(1)}s`; c.fillStyle = '#111'; const scoreSize = fitText(c, score, 590, 420); c.font = `900 ${scoreSize}px "Barlow Condensed", Impact, sans-serif`; c.fillText(score, 58, 1135);
