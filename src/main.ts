...
function renderPitConnecting() {
  ...
  screen.innerHTML = `<header class="topline"><span class="brand-mark"><span class="brand-dot"></span> Blink Tax / The Pit</span><span>Connecting…</span></header><section class="pit-connect"><div class="pit-kicker">PUBLIC ROOM · PIT</div><h2>FINDING<br>YOUR RIVALS.</h2><p>One room. Twenty-four max. Last blinker wins.</p></section>${button('Back to solo', 'secondary-button', 'pit-back')}`;
  setScreen(screen);
}
...
function renderPit(state: PitState) {
  ...
  screen.innerHTML = `<header class="topline"><span class="brand-mark"><span class="brand-dot"></span> Blink Tax / The Pit</span><span>${state.players.length}/24 · ${state.phase}</span></header><section class="pit-lobby"><div class="pit-kicker">PUBLIC ROOM · PIT</div><h2>LAST BLINKER<br>WINS.</h2><p class="pit-sub">Ranking is pure stare time. Seconds descending. No appeals.</p><div class="pit-name-row"><label for="pit-name">Your alias</label><input id="pit-name" maxlength="24" value="${escapeHtml(localStorage.getItem('blink-tax-pit-name') || '')}" placeholder="Tax Evader 7"><button data-action="pit-name">Save</button></div><div class="pit-board-head"><span>Live board</span><span>${state.phase === 'lobby' ? 'Auto-starting soon' : 'Get ready'}</span></div><ul class="pit-players">${board || '<li class="pit-empty">Waiting for blinkers to enter…</li>'}</ul>${state.phase === 'lobby' ? button('I’m ready →', 'cta pit-ready', 'pit-ready') : ''}${button('Leave the pit', 'secondary-button', 'pit-back')}</section>${countdown}`;
  setScreen(screen);
}
...
function setupPitPlay(screen: HTMLElement, state: PitState) {
  ...
  const timer = screen.querySelector<HTMLElement>('.timer')!;
  pitGame = new BlinkGame((seconds, intensity) => { timer.textContent = format(seconds); app.style.backgroundColor = `hsl(${(12 + seconds * (1.4 + intensity * 3.9)) % 360}, ${89 - intensity * 10}%, ${53 - intensity * 8}%)`; app.classList.toggle('pressure', intensity > .53); }, (seconds) => { pitDetector?.stop(); pitClient?.blink(seconds); });
...
function updatePitBoard(state: PitState) {
  ...
  const blinked = state.players.filter((player) => player.status === 'blinked');
  const count = document.querySelector<HTMLElement>('[data-pit-count]'); if (count) count.textContent = `${blinked.length} blinked`;
...
function cleanupPit() { cleanupPitPlay(); pitClient?.close(); pitClient = null; pitState = null; }
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'pit-ready') pitClient?.ready();
...
app.addEventListener('pointerdown', (event) => { const target = event.target as HTMLElement; if (game?.status === 'playing' && !target.closest('button')) game.blink(); else if (pitGame?.status === 'playing' && !target.closest('button')) pitGame.blink(); });
